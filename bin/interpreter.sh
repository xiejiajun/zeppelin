...
## TODO 解决yarn-cluster模式下 解释器进程中的的解释器生命周期管理器无法读取生命周期配置问题
#ZEPPELIN_CONF_FILES=${ZEPPELIN_HOME}/conf/zeppelin-size.xml,${ZEPPELIN_HOME}/conf/log4j_yarn_cluster.properties
#if [[ -n "${SPARK_SUBMIT}" ]]; then
#    INTERPRETER_RUN_COMMAND+=' '` echo ${SPARK_SUBMIT} --class ${ZEPPELIN_SERVER} --files ${ZEPPELIN_CONF_FILES} --driver-class-path \"${ZEPPELIN_INTP_CLASSPATH_OVERRIDES}:${ZEPPELIN_INTP_CLASSPATH}\" --driver-java-options \"${JAVA_INTP_OPTS}\" ${SPARK_SUBMIT_OPTIONS} ${ZEPPELIN_SPARK_CONF} ${SPARK_APP_JAR} ${CALLBACK_HOST} ${PORT} \"${INTP_GROUP_ID}\" ${INTP_PORT}`
#else
...
## https://github.com/apache/zeppelin/pull/3925
